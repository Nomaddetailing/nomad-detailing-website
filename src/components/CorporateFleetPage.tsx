...
const scrollToQuote = () => {
  ...
  const [submitting, setSubmitting] = useState(false);
    ...
    const submitFleet = async (e: React.FormEvent) => {
        ...
        try {
            const endpoint =
                (import.meta.env.VITE_FLEET_ENDPOINT as string | undefined) || "/api/fleet";
            ...
            const resp = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    ...
                    service_frequency: form.serviceFrequency, // now lowercase from dropdown
            ...
            if (!resp.ok || !json?.ok) {
                // show a real error instead of fake success
                throw new Error(json?.errors?.join(", ") || json?.error || `HTTP ${resp.status}`);
            }
            ...
            alert(`Submission failed: ${String((err as any)?.message || err)}`);
        } finally {
            setSubmitting(false);
        }
    };
